...
base_path = r"C:\Users\tv239\Downloads\FINAL_CODE\\"
...
def insert_data_to_db(data):
    ...
    try:
        ...
        formatted_data = [(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9]) for row in data]
...
def main_function():
    ...
    image_path2 = base_path + r"top.jpg"
    ...
    new_data.append([partno, second_partno, statuses[0], statuses[1], nb_status, rr_status, formatted_datetime, "eclip.jpg", "drivescrew.jpg", "nbbr.jpg"])
    ...
    destination_folder_2_quality = base_path + r"quality_check_prediction_folder"
    ...
    for filename in os.listdir(source_folder):
        ...
        if os.path.isfile(source_image_path) and filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            destination_image_path = os.path.join(destination_folder_2_quality, new_filename)
            shutil.copy(source_image_path, destination_image_path)
            print(f"Copied {filename} to {destination_folder_2_quality}")

    print("All images copied successfully to quality folder.")
    et2 = time.time()
...
def home():
    ...
    return render_template('index_2.html', data=data, enumerate=enumerate, model_type=model_type)
...
@app.route('/view_images/<part_no>')
def view_images(part_no):
    data = fetch_data_from_db()
    for row in data:
        if row['part_no'] == part_no:
            return render_template('view_images.html', row=row)
    return "Part number not found", 404
